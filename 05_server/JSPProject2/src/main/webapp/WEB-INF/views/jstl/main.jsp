...
<%-- 
    ...
    tablib : 태그 라이브러리 추가(JSTL, 커스텀태그)

    prefix : 접두사(앞에 붙이는 말, 단어)
            태그명 앞에 작성되는 단어 <c:if>

    uri(Uniform Resource Identifier, 통합 자원 직별자) 
        -> 자원을 식별하는 고유 문자열 (아이디 같은 개념(중복 X))

    (참고) url(Uniform Resource Locatior)
            -> 자원의 위치를 나타내는 문자열 (경로)
--%>
...
<body>
    ...
    <pre>
        ...
        표준으로 제공하는 라이브러리 
        (if, for 간단히 쓰고 싶으면 이 라이브러리 사용해라)
    </pre>
