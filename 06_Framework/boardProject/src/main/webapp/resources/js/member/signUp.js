...
// 특징
// 1) 원하는 value(V)를 얻어오는 방법
//    - 객체명.Key
//    - 객체명["Key"]

// 2) 객체에 특정 Key(필드)가 존재하지 않으면 추가할 수 있다
// ex) const obj = {"a":1, "b":2}
//     obj.c = 3  // -> {"a":1, "b":2, "c":3}

// 3) 객체에 특정 Key를 삭제할 수 있다
// ex) const obj = {"a":1, "b":2}
//     delete obj.b;  // {"a":1}
    ...
    if( regEx.test(memberEmail.value) ){ // 유효한 경우
        ...
        .then(count => {
            ...
            if(count == 0){
                ...
                emailMessage.classList.remove("error"); // css에 .error 스타일 제거
                checkObj.memberEmail = true; // 유효 O
            
            }else{
                emailMessage.innerText = "이미 사용 중인 이메일 입니다"
                emailMessage.classList.add("error"); // css에 .error 스타일 적용
                emailMessage.classList.remove("confirm"); // css에 .confirm 스타일 제거
