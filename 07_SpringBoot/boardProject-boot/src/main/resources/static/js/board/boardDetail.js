...
boardLike.addEventListener("click", e => {
    ...
    if(e.target.classList.contains("fa-regular")){ // 좋아요 X(빈 하트)
        check = 0;
    ...
    // ajax로 서버로 제출할 파라미터를 모아둔 JS 객체 (K:V)
    ...
    .then(response => response.text()) // 응답 객체를 필요한 형태로 파싱하여 리턴 ({return ~} 생략)
...
document.getElementById("updateBtn").addEventListener('click', () => {
    ...
    // /board/2/2006 --> /board2/2/2006/update?cp=1 (GET)
});
...
document.getElementById("deleteBtn").addEventListener("click", () => {
    ...
    if(confirm("정말 삭제하시겠습니까?")){
        ...
        // /board/2/2006 --> /board2/2/2006/delete (GET)    

        // 삭제 서비스 호출 성공 시 redirect:/board/{boardCode}
        // + RedirectAttributes 이용해서 "삭제되었습니다" alert 출력
