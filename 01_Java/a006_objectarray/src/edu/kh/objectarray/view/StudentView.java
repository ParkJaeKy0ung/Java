...
public class StudentView {
	...
	private void selectAll() {
		...
		// 배열! 을 반환 받았으므로 배열로 받아야 함.
		Student[] studentList = service.selectAll(); 
		// studentArr의 주소를 studentList에 대입 <- Student[]을 참조하는 studentArr의 주소
		// *********************************************
		// 매개변수, 반환 값으로 참조변수가 사용될 경우
		// 참조하고 있는 주소가 복사됨! ==> 얕은 복사
		// *********************************************
		
		// 배열 안에 있는 값들 출력 위한 for문
		for(int i=0; i<studentList.length; i++) {
			
			if(studentList[i] == null) {
				break; // 참조하는 객체가 없으면 반복 종료
				// studentList[i]가 비어있으면 i+1도 비어있는 거
				// break;
			}
			...
			// NullPointException
			// -> 참조하고 있는 객체가 없는 (studentList[i]==null)
			// 	  참조변수를 이용해서
			// 	  필드/메서드를 호출할 때 발생하는 에러(오류)
	...
	private void selectIndex() {
		...
		// 반환 받은 index번째 요소가 null이 아닐 경우(학생 정보 있을 경우)
		...
		// System.out.println(std.toString()); -> null 상태
		// 해결방법 : if문 사용해서 null이 아닐 때 출력하도록
	}
