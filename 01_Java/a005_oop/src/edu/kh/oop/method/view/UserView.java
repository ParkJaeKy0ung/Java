...
public class UserView {
	...
	// UserService 객체 생성
	private UserService service = new UserService(); // 기본 생성자
	...
	public static User loginUser = null;  // static은 프로그램 키자마자 제일 먼저 메모리에 저장! main 메서드보다도 먼저 생성됨.
...
//	}
	...
	private void login() {
		...
		if(UserView.loginUser != null) { // 성공
			System.out.println(UserView.loginUser.getUserName() + "님 환영합니다."); // getter
		}else { // 실패
	...
	public void userUpdate() {
		...
		if(UserView.loginUser != null) { // 로그인 O
			...
			// -> true/false (논리값 반환)
			if(service.userUpdate(userName, userGender, userPw)) { // 수정 성공 시
