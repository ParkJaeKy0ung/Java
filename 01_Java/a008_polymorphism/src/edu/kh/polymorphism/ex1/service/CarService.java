...
//		c1.getDiscountOffer();
	...
	public void ex3() {
		...
		for(Car c : carList) {
			...
			if(c instanceof Truck) {
				...
				// if(c instanceof Truck) 를 쓰지 않으면 Truck <-> LightCar 충돌
				// 만약 다운캐스팅이 잘못될 경우
				// ClassCastException이 발생한다!
				// ex) LightCar 객체를 Truck 참조변수 참조하려고 할 때
				
				// 해결 방법 : instanceof를 통해 참조하는 객체의 자료형을 판단하여
				// 			   적절한 형식으로 다운캐스팅 진행
				...
				// c가 참조하는 객체가 Truck, LightCar가 아닐 때 (그냥 Car일 때)
	...
	public void ex4() {
		...
		// carList[i] == Car 참조변수
		carList[0] = new Truck(6, 4, "경유", 1); // 업캐스팅
					 // -> 동적 바인딩 : Truck의 toString이 실행됨.
		carList[1] = new LightCar(4, 4, "휘발유", 0.3); // 업캐스팅
					 // -> 동적 바인딩 : LightCar의 toString이 실행됨.
		carList[2] = new Car(4, 2, "전기");
					 // -> 동적 바인딩 : Car의 toString이 실행됨.
		
		for(Car c : carList) {
			
//			printCar(Car객체 주소 또는 Car를 상속받은 객체 주소);
			printCar(c);
	...
	public void printCar(Car c) {
		...
		// -> 오버라이딩 우선권!(자식이 부모에 우선함)이 동적 바인딩 때문에 발생하는 것
	}
