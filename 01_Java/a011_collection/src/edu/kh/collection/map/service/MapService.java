...
public class MapService {
	...
	public void ex1() {
		...
		System.out.println(map); // {1=김밥, 2=라면, 3=떡볶이, 4=돈까스}
								 // Integer의 특징으로 순서 유지됨
		
		// Key가 중복되는 경우 -> 덮어 씌움
		map.put(4, "치즈 돈까스");
		System.out.println(map); // {1=김밥, 2=라면, 3=떡볶이, 4=치즈 돈까스}
...
//		Map<String, Object> member = new HashMap<>();
		...
		//    == 모든 객체의 부모 타입의 참조변수로 사용 가능(다형성 업캐스팅)
		//    == value에 어떤 객체든 작성 가능
		...
		member.put("memberAge", 23); // AutoBoxing 적용되어 Integer
		member.put("memberGender", 'M'); // AutoBoxing
		...
		if(member.get("memberId").equals(id)) {
			...
			if(member.get("memberPw").equals(pw)) {
				...
				// 아이디와 비밀번호 모두 일치할 때 member의 모든 정보 출력
