...
public class ListService {
	 ...
	 * -> ArrayList(최신버전), Vector(옛날버전), LinkedList
	 * */
...
//		ArrayList list = new ArrayList(); // 기본 생성자 -> 10칸짜리 생성
		...
		// void add(int index, E e) : 원하는 idex 위치에 요소 추가
		list.add(1, "중간 삽입");
		
		// list.add(10, "삽입");  // 인덱스 범위 넘어섬. 오류 발생
		
		// E get(int index) : 주어진 index의 요소를 반환
		// int size() : 리스트 크기 X, 리스트에 저정된 요소의 개수 (=length)
		System.out.println(list.get(3)); // 3.14
	...
	public void ex2() {
		...
		// 컴파일 단계에서 타입 체크 
		// -> 컬렉션에 사용되는 객체의 타입이
		// 	  컬렉션 생성 시 지적된 타입과 같은지 확인
		// 	  + 자동으로 다운캐스팅 진행
		// ex1()에서는 list.add -> Object 타입, ex2()에서는 list.add -> String
