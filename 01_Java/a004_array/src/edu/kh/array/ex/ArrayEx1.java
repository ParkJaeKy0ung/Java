...
public class ArrayEx1 {
	...
	public void ex1() {
		...
		// 배열 선언
		int[] arr;
		// - int 배열을 참조(저장X)할 공간 할당하고 이를 arr 이라고 부르겠다.
		// * 기본 자료형 8개를 제외한 나머지는 다 "참조형" 이라고 함. int[] -> 참조형
...
//		String [] menuArr = new String[10];
		...
		System.out.println("오늘의 점심 메뉴는? : " + menuArr[index]);
	}
	...
	public void ex5() {
		...
		int[] arr = {10, 30, 50, 70, 90};  // index : 0,1,2,3,4
		...
		for(int i=0; i<=arr.length; i++) {  // i = 0,1,2,3,4,5
			...
			// ---------> 이런 식의 시험(문제해결 시나리오), 면접!!! 오류 찾아낼 줄 알아야 함!!!
			// 문제점 : (원인) for문의 조건식에서 i의 범위가 
			// 			arr 배열의 index 범위를 초과하는 값까지 
			//			증가하도록 작성되어
			//			(어디에서) 실행 시 for문 내 출력 구문에서
			//			(무엇이) ArrayIndexOutOfBoundsException이 발생함.
			
			// 해결방법 : 조건식을 i < arr.length로 수정하여
			// 			  i가 배열의 index 범위를 초과하지 않도록 함.
		}
	}
	...
	public void ex6() {  // 다시 !!!
		...
		int[] scoreArr = new int[input]; // 입력 받은 정수의 길이를 갖는 배열
		...
		for(int i=0; i<input; i++) {
			...
			sum += scoreArr[i]; // nextInt 입력하지 않게 주의해야 함. 입력X
		...
		for(int i=0; i<scoreArr.length; i++) {
			...
			// 0번 1번 index 중 큰 값 > scoreArr[2]  --> 0번 1번 중 큰 값 기억하는 법 : 메모리에 저장 = 변수 생성
...
//		}
	...
	public void ex8() {
		...
		copyArr1[0] = 9999;  // arr1과 copyArr1은 같은 배열을 참조하고 있기 때문에 같이 변경됨.
		...
		// for문을 이용해서 깊은 복사  --> 이걸 주로 사용함
//		for(int i=0; i<arr2.length; i++) {
//			copyArr2[i] = arr2[i];
//		}
		
		// System.arraycopy()를 이용한 깊은 복사  --> 참고 정도로만 알고 있으면 됨. 
		// 설명 보고 싶으면, Shift + f2
