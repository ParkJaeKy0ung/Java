...
public class APIService {
	...
	public APIService() {
		...
		studentList[2] = new Student(3, 4, 10, "박민지");
	}
...
//		String[] names = input.split("/");
		...
		for(String n : names) {
			...
			for(Student s : studentList) {
				...
				if(s.getName().equals(n)) {
					...
					result[index] = s; // result 배열에 일치하는 학생의 주소 저장 (얕은 복사)
					index++;
				}
			}
			//**
	...
	public String printName() {
		...
		for(Student s : studentList) {
			...
			size++; // 학생 3명 있으므로 if(s == null)는 다 통과하고 size는 3이 됨.
		}
