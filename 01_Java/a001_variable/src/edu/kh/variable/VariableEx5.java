...
public class VariableEx5 {
		...
		public static void main(String[] args) {
			...
			System.out.println(ch5 + " Ucicode Number : " + num5);
			...
			byte test2 = (byte)test1; //Type mismatch: cannot convert from int to byte
			              // 변환시키고자 하는 자료형을 명시해서 강제 변환
			
			System.out.println("test1 : " + test1);
			System.out.println("test2 : " + test2);
			
			// ctrl + alt + 방향키 위/아래 : 라인 복사
			// 화면 뒤집힐 경우! 인텔HD그래픽 → 그래픽 옵션 → 바로가기 키 → 사용 안 함
			...
			System.out.println("(int) 1/2 = " + test5 / test6);
							//   1   /   2   =  0.5 (double)
							//  int  / int   =  int (컴파일러가 강제 형변환)
			
			// 자동 형변환 + 강제 형변환
			System.out.println( "double로 강제 형변환 : 1/2 = " + (double)test5 / test6 );
							   // 강제 형변환
							   // -> 1.0(double) / 2(int)
							   // 				  -> 2.0(double) 자동 형변환
			...
			bNum = (byte)(bNum + 1);
			//	   (byte)((int)128)
			System.out.println(bNum); // 128 -> -128 (오버플로우 발생)
			...
			//System.out.println(iNum3); // 해결방법 long으로 변환
