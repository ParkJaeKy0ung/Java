...
public class ConditionEx {
	...
	*/
	...
	public void ex3() {
		...
		// 홀수, 짝수, 0
		if(input == 0) {                      //  0
			System.out.println("0 홀/짝을 구분 할 수 없습니다.");
	...
	// if 예시 4번 (if - else if - else)
	public void ex4() {
		Scanner sc = new Scanner(System.in);
		
		System.out.print("계절이 알고싶은 달(월)을 입력하세요 : ");
		int input = sc.nextInt();
		
		// 조건문 결과를 저장할 문자열 변수 선언
		String result;
		
		if(3 <= input && input <= 5 ) { // 3, 4, 5
			// input == 3 || input == 4 || input == 5  // 길수록 안 좋은 코딩
			result = "봄";
		}else if(6 <= input && input <= 8) { // 6, 7, 8
			result = "여름";
		}else if(9 <= input && input <= 11) { // 9, 10, 11
			result = "가을";
	...
	public void ex5() {
		...
		if( i <= 13) {
			result = "어린이";
		}else if( i <= 19 ) {  // 13 < i && i <= 19 : 중첩된 코드! 90점짜리 코딩
			result = "청소년";
		}else {
			result = "성인";
		}
