...
public class MainDAO {
	...
	public MainDAO() { // 기본 생성자
		...
		// DAO 객체가 생성될 때 외부에 작성된 XML 파일을 읽어와 Properties 객체에 저장
		try {
			prop = new Properties();
	...
	public Member login(Connection conn, String memberId, String memberPw) throws Exception {
		...
		try {
			// 2. SQL 작성 후 수행
			String sql = prop.getProperty("login"); // key값(login)을 불러와 SQL 조회되도록?
			
			// System.out.println(sql); --> 제대로 불려왔는지 확인하는 출력구문
