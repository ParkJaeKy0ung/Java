...
public class ForEx {
	...
	public void ex4() {
		...
		int sum = 0; // i가 증가하면서 변한 값들을 누적할 변수
					 // 0으로 초기화 하는 이유 : 어떤 값을 더하든 영향이 없기 때문  **곱할 때는 초기 값 1 !!!
		for(int i = 1 ; i<=100 ; i++) {
			//sum = sum + i;
			sum += i;
		}
	...
	public void ex5() {
		...
		// 	sum = i1 + i2;
		// }
		// System.out.println("2부터 5까지 모든 정수의 합 : " + sum);
	...
	public void ex6() {
		...
		// 1) A(65), Z(90)의 유니코드(아스키) 번호를 이용하기  // 코드 번호 모르면 사용 불가
		for(int i = 65 ; i <= 90 ; i++) {
			System.out.print((char)i);
		}
		
		// 2) 유니코드 번호를 모를 때  // 강제 형변환 번거로움
		System.out.println();
		for(int i = 'A'; i <= 'Z' ; i++) {
			System.out.print((char)i);  // 그냥 i로 하면 해당하는 번호 출력됨.
		}
		
		// 3) char 자료형은 문자형이지만 실제로 정수를 저장한다  // 가장 이상적
		System.out.println();
		for(char i = 'A' ; i <= 'Z' ; i++) {
			System.out.print(i);
		}
	...
	public void ex9() {
		...
		for(int i=1; i<=input; i++) {
			System.out.printf("입력 %d : %d", i, sum);
			sum += i;
		}
	...
	// for문 응용 사용법 8  *** 오답 !!! *** if문 사용할 때 정상 출력되는 부분을 else로 하는 게 더 좋음
	public void ex14() {
		// 입력 받은 단의 구구단 출력하기
	...
	public void ex16() {
		...
		//  2  4  6  8 10  2*1 2*2 2*4 2*5 2*5
	...
	public void ex17() {
		...
		for(int dan=2; dan<=9; dan++) {
			for(int x=1; x<=9; x++) {
				System.out.printf("%dx%d=%d ", dan, x, dan*x);
			}
			System.out.println();
		}
	}
	...
	public void ex18() {
		...
		for(int x=1; x<=4; x++) { //  4줄 반복 (x=1,2,3,4)
			for(int i=1; i<=x; i++) {  // 한 줄 출력  // x자리가 1,2,3,4로 바뀌어야 하는데 x값이 1,2,3,4 !
				System.out.print(i);
			}
