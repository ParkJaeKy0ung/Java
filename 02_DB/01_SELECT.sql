...
--> 테이블 정보(ctrl - 테이블명 클릭)에서 컬럼명, COMMENT 확인
...
-- 컬럼 값 : 행과 열이 교차되는 테이블의 한 칸(== 한 셀(엑셀에서))에 작성된 값(DATA)
...
-- EMPLOYEE 테이블에서 이름, 입사일, 오늘까지 근무한 날짜
...
SELECT EMP_NAME, SALARY, '원' 단위  -- 별칭 사용! (= AS 단위)
FROM EMPLOYEE;

----------------------------------------------------------

-- <DISTINCT> (별개의, 따로따로)
--> 조회 시 지정된 컬럼에 중복 값을 한 번만 표시
...
-- EMPLOYEE 테이블에서 
-- 연봉이 5천만원 이하인 사원의 사번, 이름만 조회
/*3*/ SELECT EMP_ID, EMP_NAME
/*1*/ FROM EMPLOYEE 
/*2*/ WHERE SALARY * 12 <= 50000000; 
-- SELECT절에 SALARY가 없는데도 정상 처리됨 => WHERE절 먼저 수행됨을 알 수 있음
...
-- EMPLOYEE 테이블에서
-- 급여가 300만 이상 500만 이하인 직원의 모든 정보 조회
SELECT *
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서
-- 급여가 300만 이상 500만 이하가 아닌 직원의 모든 정보 조회
-- == 300만 미만, 500만 초과

-- NOT BETWEEN A AND B : 컬럼 값이 A 이상 B 이하가 아닌 경우
-- == A 미만, B 초과
SELECT *
FROM EMPLOYEE
...
-- 컬럼명 NOT IN(값1, 값2, 값3, ...) : ()내 값을 제외한 행 TRUE
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
