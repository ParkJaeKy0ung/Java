 ...
 * 	  - 모든 권한과 책임을 가지는 계정
 * 		ex) SYS(최고 관리자), SYSTEM(SYS에서 권한 몇 개가 제외된 관리자)
 * 
 * 2. 사용자 계정
...
-- 1. 사용자 계정 생성
-- CREATE USER 사용자명 IDENTIRIED BY 비밀번호;
CREATE USER pjk_sample IDENTIFIED BY "sample1234";
									  --> 대소문자 구분
...
GRANT CREATE SESSION TO pjk_sample;
...
-- 6. 테이블 생성 권한 부여
--    + 객체 생성 공간 할당
GRANT CREATE TABLE TO pjk_sample;

ALTER USER pjk_sample DEFAULT TABLESPACE SYSTEM 
...
GRANT SELECT ON EMPLOYEE TO pjk_sample;

----------------------sample 계정으로 접속----------------------
-- 3. 다시 kh 계정의 EMPLOYEE 테이블 조회
SELECT * FROM KH_PJK.EMPLOYEE;

-- 4. 선동일 삭제
DELETE FROM KH_PJK.EMPLOYEE WHERE EMP_NAME = '선동일';
-- ORA-01031: 권한이 불충분합니다

----------------------KH 계정으로 접속----------------------
-- 5. sample 계정의 EMPLOYEE 테이블 조회 권한 회수
REVOKE SELECT ON EMPLOYEE FROM pjk_sample;
...
GRANT
	...
	CREATE TYPE
	CREATE TRIGGER
	CREATE TABLE TO pjk_sample;

--> ROLE을 이용해서 간단히 부여 / 회수
GRANT RESOURCE TO pjk_sample;
REVOKE RESOURCE FROM pjk_sample;

-- 사용자에게 부여된 시스템 권한 조회
-- SELECT * FROM DBA_SYS_PRIVS 
-- WHERE GRANTEE = 'PJK_SAMPLE';
