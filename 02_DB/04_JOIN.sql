...
-- 기존 INNER JOIN 결과(21행)에
-- JOIN 구문 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서 
-- JOIN 결과에 포함되지 않은 행들도 조회하도록 함.(23행)
...
WHERE DEPT_CODE = DEPT_ID(+);
-- NULL = D1 ~ D9(+)
-- 왼쪽 테이블 컬럼의 값과, 오른쪽 테이블 컬럼의 값이 일치하지 않아도 억지로 추가해라
...
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- 23(인원 수) * 9(부서) = 207행
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> 하나의 테이블 가지고 생각하지 말고 같은 테이블이 2개가 있다고 생각하면 쉬움

-- EMPLOYEE 테이블에서 사번, 이름, 사수사번, 사수이름 조회

-- ANSI 표준
SELECT EMP_ID, EMP_NAME, MANAGER_ID, EMP_NAME
FROM EMPLOYEE
...
-- LEFT 를 붙이면 MANAGER_ID가 NULL인 값까지 나옴.
...
-- NATURAL JOIN이 성립되지 않는 경우(같은 컬럼이 없는 경우) 
--> CROSS JOIN 결과가 조회됨.
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME -- 누가 어느 부서 어디에 있는가
FROM EMPLOYEE 
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
...
/*
SELECT SAL_LEVEL, EMP_NAME, JOB_NAME, SALARY, SALARY*(SALARY+NVL(BONUS, 0)) 연봉
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
GROUP BY SAL_LEVEL, EMP_NAME, JOB_NAME, SALARY, SALARY*(SALARY+NVL(BONUS, 0))
HAVING MIN(SAL_LEVEL) < SALARY*(SALARY+NVL(BONUS, 0))
ORDER BY EMP_ID;
*/

/* ?????????
SELECT SAL_LEVEL, EMP_NAME
FROM EMPLOYEE
GROUP BY SAL_LEVEL, EMP_NAME;
*/
...
WHERE EMP.EMP_ID != LOYEE.EMP_ID -- 자기 자식 동료 X 삭제
ORDER BY 1;
