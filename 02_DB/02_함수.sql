...
SELECT 123.456, 
	...
	ROUND(123.456, -1), --120    (소수점 -1번째 자리 -> 정수 1의 자리에서 반올림)
	ROUND(123.456, -2)  --100    (소수점 -2번째 자리 -> 정수 10의 자리에서 반올림)
FROM DUAL;
...
SELECT FLOOR(123.5), TRUNC(123.5) --123, 123
FROM DUAL;

SELECT FLOOR(-123.5), TRUNC(-123.5) -- -124, -123 (내림, 버림의 차이 주의)
FROM DUAL;


SELECT EMP_NAME, TRUNC(SALARY, -6) || '원 이상' 급여 
				 -- 10만 자리까지 잘라서 100만 단위 너남음
FROM EMPLOYEE;
...
-- 2023-03-07 12:22:45.000 , 2023-03-07 12:22:45.106 +0900
...
-- EXTRACT(TEAR | MONTH | DAY FROM 날짜)
 ...
 * 2) 0 : 숫자 한 칸을 의미, 오른족 정렬, 빈 칸에 0을 추가 
 * 3) L : 현재 시스템이나 DB에 설정된 나라의 화폐 기호
 * 4) , : 숫자의 자릿수 구분
 *  */

SELECT 1234, 
	TO_CHAR(1234, '99999'), --_1234
...
-- EMPLOYEE 테이블에서 
-- 사번, 이름, 연봉(|100,000,000 형식으로 조회) 조회
...
/* 날짜 패턴 Y, R의 차이점 */
-- 년도를 짧게 해석하는 경우
-- 50 미만 : Y, R 둘 다 앞부분에 현재 세기(21C == 2000년대)를 적용
-- 50 이상 : Y == 현재 세기(2000년대)
--   	    R == 이전 세기(1900년대)

-- 1949-01-15
-- 1950-01-15
SELECT TO_DATE('490115', 'YYMMDD'), --2049-01-15 00:00:00.000
	   TO_DATE('490115', 'RRMMDD'), --2049-01-15 00:00:00.000
	   
	   TO_DATE('500115', 'YYMMDD'), --2050-01-15 00:00:00.000
	   TO_DATE('500115', 'RRMMDD')  -- 1950-01-15 00:00:00.000
FROM DUAL;
...
SELECT TO_NUMBER('$1,500', '$9,999') FROM DUAL; --1,500
...
-- DECODE(계산식 | 컬럼명, 조건1, 결과1, 조건2, 결과2, ... [, 아무것도 일치하지 않을 때])
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 성별 조회하기
SELECT EMP_ID, EMP_NAME, 
...
-- DECODE는 계산식|컬럼명이 딱 떨어지는 경우에만 사용 가능
-- CASE는 계산식|컬럼값을 범위로 지정할 수 있다.
...
SELECT MIN(SALARY), MAX(SALARY),   -- 숫자 : 대/소
	MIN(EMP_NAME), MAX(EMP_NAME),  -- 문자열 : 문자 순서
	MIN(HIRE_DATE), MAX(HIRE_DATE) -- 날짜 : 과거 < 미래
FROM EMPLOYEE;


-- COUNT(* | 컬럼명) : 조회된 행의 개수를 반환
-- COUNT(*)     : NULL을 포함한 모든 행의 개수 반환
-- COUNT(컬럼명) : 지정된 컬럼의 값이 NULL인 경우를 제외한 행의 개수 반환
-- COUNT(DISTINCT 컬럼명) : 지정된 컬럼에서 중복된 값을 제외한 행의 개수 반환
...
WHERE DEPT_CODE IS NOT NULL; --이렇게도 가능함

-- EMPLOYEE 테이블에 존재하는 직급코드의 개수
SELECT COUNT(DISTINCT JOB_CODE) FROM EMPLOYEE; --7(중복 제거)

-- EMPLOYEE 테이블의 남자 사원 수
SELECT COUNT(*) FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = '1';

--EMPLOYEE 테이블의 여자 사원 수
SELECT COUNT(*) FROM EMPLOYEE
