...
-- 			(주의사항!) ORDER BY절 해석 전에 먼저 행번호를 부여함
SELECT /*ROWNUM,*/ EMP_ID 
FROM (SELECT EMP_ID FROM EMPLOYEE ORDER BY SALARY);
WHERE ROWNUM = 1; 

/*ROWNUM 사용 주의사항*/
--SELECT ROWNUM, EMP_ID, SALARY
--FROM EMPLOYEE
--ORDER BY SALARY;

--> 확실한 방법(복잡하지만 신뢰도 있는 정보 조회)
-- 메인쿼리 + 서브쿼리(WHERE) + 서브쿼리(FROM)
SELECT EMP_ID, EMP_NAME, JOB_CODE, DEPT_CODE, SALARY, HIRE_DATE
FROM EMPLOYEE
WHERE EMP_ID = (SELECT /*ROWNUM,*/ EMP_ID 
...
SELECT EMP_NAME, DEPT_CODE -- 한국에서 근무하는 사원 조회
FROM EMPLOYEE
...
-- (메인 쿼리 각 행 마다 : 메인 -> 서브 -> 메인)


-- 사수가 있는 직원의 사번, 이름, 부서명, 사수사번 조회
-- EXISTS : 서브퀴리에서 조회된 결과와 일치하는 행이
...
/* 이 방법으로 안 되는 이유는 왜일까...
SELECT JOB_CODE, TRUNC(AVG(SALARY), -4) "직급별 급여 평균"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
SELECT EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > SELECT JOB_CODE, TRUNC(AVG(SALARY), -4)
			   FROM EMPLOYEE
			   GROUP BY JOB_CODE
			   ORDER BY JOB_CODE;
*/
...
-- 1) 간단하지만 신뢰도 낮은 결과 (날짜가 겹칠 경우 데이터 어쩌구 될 가능성 있음)
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE /*DEPT_CODE,*/ HIRE_DATE 
					 IN (SELECT /*DEPT_CODE,*/ MIN(HIRE_DATE) FROM EMPLOYEE
						 WHERE ENT_YN = 'N'
						 GROUP BY DEPT_CODE)
ORDER BY HIRE_DATE;
/*DEPT_CODE, : 사용하면 NULL값이 출력되지 않음. IN은 = OR를 반복하므로 NULL 값 처리 X*/
...
WHERE HIRE_DATE 
	  = (
		 ...
		 WHERE (ENT_YN = 'N' AND SUB.DEPT_CODE = MAIN.DEPT_CODE) -- NULL이 아닌 경우 6행
		 OR (ENT_YN = 'N' 
		 	 AND MAIN.DEPT_CODE IS NULL
		 	 AND SUB.DEPT_CODE IS NULL) -- NULL인 경우 1행(하동운)
		 )
ORDER BY HIRE_DATE;
...
WHERE HIRE_DATE 
	  = (
	  	 SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB
		 WHERE ENT_YN = 'N' 
		 AND NVL(SUB.DEPT_CODE, 1) = NVL(MAIN.DEPT_CODE, 1)
		 )
ORDER BY HIRE_DATE;
...
-- EX) 게시판에 좋아요 수, 댓글 수, 신고 수 등등 조회할 때 많이 사용함!!!
SELECT EMP_NAME, JOB_CODE, SALARY FROM EMPLOYEE; -- 메인쿼리에서 JOB 코드 얻어와서
												 -- 서브쿼리에서 비교
SELECT EMP_NAME, JOB_CODE, SALARY,
...
/*1*/ FROM EMPLOYEE
/*2*/ WHERE ROWNUM <= 5; -- 유재식 : 5번

-- * 조건절에 ROWNUM 사용 시 주의사항!
-- -> 조건절에 RUWNUM을 작성하는 경우 
--    반드시 조건 범위 내에 1 (1번 행)이 포함되어야 한다.
SELECT EMP_NAME, ROWNUM
FROM EMPLOYEE
WHERE ROWNUM = '유재식'; -- 유재식 : 1번
/*WHERE ROWNUM = 5; -- XXX*/
...
ORDER BY SALARY DESC;
-- 선동일 송종기 정중하 대북혼 노옹철
--> EMPLOYEE 테이블 위에서부터 5명을 가지고 급여 내림차순 조회

-- 문제 원인 : SELECT 해석 순서 상 
--			 ORDER BY가 가장 마지막으로 해석되기 때문에
--			 급여 상위 5명이라는 의도와 맞지 않는 형태로 조회됨

-- 해결방법 : 인라인뷰(FROM절에 작성되는 서브쿼리)를 이용
-- 해결 1) 급여 내림차순 조회 
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;
...
-- ** 인라인뷰 사용 시 컬럼명에 유의해야 한다! ** 
	  --> 메인쿼리에서 인라인 뷰의 컬럼명을 그대로 사용해야 한다!
-- 급여 평균이 590만인 부서를 조회
/* */ SELECT *
/*1*/ FROM (SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY)) 평균
		    FROM EMPLOYEE
		    LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
		    GROUP BY DEPT_CODE, DEPT_TITLE
		    ORDER BY 평균 DESC)
