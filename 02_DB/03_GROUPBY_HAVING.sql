...
/*3*/ SELECT DEPT_CODE, SUM(SALARY) -- 3) 그룹별 SALARY 합계를 조회
...
-- 직급코드 오름차순
SELECT JOB_CODE, COUNT(BONUS) "보너스 받는 사원 수" 
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;

 
-- 성별, 성별 별 급여 평균(내림처리), 급여 합, 인원 수 조회
-- 인원 수 내림차순 정렬 
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별,
	   FLOOR(AVG(SALARY)) "급여 평균", SUM(SALARY) "급여 합", COUNT(*) "인원 수"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 함수 입력 가능!
	--> SELECT절 해석이 되지 않아 성별 별칭 사용 불가능
ORDER BY "인원 수";


-- WHERE + GROUP BY 
--> WHERE절이 GROUP BY절보다 우선순위가 빠르다.

-- EMPLOYEE 테이블에서 부서코드가 'D5', 'D6'인 부서의 평균 급여 조회
SELECT DEPT_CODE, ROUND(AVG(SALARY)) --4) DEPT_CODE와 급여 평균 조회
