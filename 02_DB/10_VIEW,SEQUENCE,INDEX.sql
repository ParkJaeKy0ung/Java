...
--> VIEW는 값을 저장하지 않는다고 했는데 저장된 것처럼 보임!
--   실제 VIEW에 저장된 것이 아니라 
...
--> VIEW에 적었지만 원본 테이블 DEPT_ID, LOCATION_ID 컬럼에 'D0', 'L3' 삽입됨
---> DEPT_TITLE 컬럼에는 NULL 삽입
--   BUT, DEPT_TITLE에는 NOT NULL 제약조건이 설정되어 있어
--   NULL 값이 삽입될 수 없음.

-- ORA-01400: NULL을 ("KH_PJK"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
...
/* SEQUENCE(순서, 연속)
 * - 순차적으로 일정한 간견의 숫자(번호)를 발생시키는 객체
 *   (번호 생성기)
 * 
 * *** SEQUENCE 왜 사용할까? ***
 ...
 *   [작성법]
  ...
  [CACHE 바이트크기 | NOCACHE] -- 캐쉬메모리 기본값은 20바이트, 최소값은 2바이트
	...
	-- 시퀀스의 캐쉬 메모리는 할당된 크기만큼 미리 다음 값들을 생성해 저장해둠
...
-- UPDATE 확인
SELECT * FROM TB_TEST; -- 고길동 140 -> 고길동 145
...
-- UPDATE 확인
SELECT * FROM TB_TEST; -- 고길동 145 -> 고길동 150
 ...
 * 
 * ** INDEX의 단점 ** 
 * - 데이터 변경(ISERT, UPDATE, DELETE) 작업 시 이진 트리 구조에 변형이 일어남
 *   -> DML 작업이 빈번한 경우 시스템 부하가 늘어 성능 저하됨.
 * 
 * - 인덱스도 하나의 객체이기 때문에 별도 저장공긴이 필요 (메모리 소비)
 * 
 * - 인덱스 생성 시간이 필요함.
 * 
 * [작성법]
...
WHERE TEST_ID = 'TEST500000'; -- 40/30/36/32 ms

-- INDEX 사용 O 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; -- 1/0/2 ms
...
-- 인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 10/1/1/1 ms
...
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 32/36 ms
