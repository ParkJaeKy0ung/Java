...
function accessId(){  // 함수 선언
    ...
    if(bgColor == "red"){  /* JS는 문자열 비교 시에도 비교 연산자 사용 */
...
function accessName(){
    ...
    for(var i=0; i<hobbyList.length; i++){
        ...
        if(hobbyList[i].checked){ // 체크되어 있을 경우
...
function accessTagName(){
    ...
    // 각 li 요소에 작성된 색으로 글자 색 변경
    for(var i=0; i<arr.length; i++){
        arr[i].style.color = arr[i].innerText; /* value(X) innerText(O)*/
    }
}
...
function cssTest(){
    ...
    // 1개만 선택되는 요소
    document.querySelector("#cssDiv").style.border = "3px solid green";
