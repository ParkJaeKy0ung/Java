...
const TodoList = () => {
    ...
    // 전역변수 useContext를 씀, context는 TodoList, 거기에서 todoList, setTodoList, loginMember 얻어옴 ???
    ...
    const handleDeleteTodo = (todoNo, index) => {
        ...
        .then(result => {
            ...
            // 배열.splice(인덱스, 몇칸)
            // -> 배열의 인덱스 번째 요소부터 몇 칸을 잘라내서 반환할지 지정
            // --> 배열에서 잘라내진 부분이 사라짐
    ...
    return (
        <>
            {/* loginMember는 TodoListContext에서 꺼내옴 */}
