...
public class EmpView {
	...
	private void selectRetirement() {
		...
		try {
			List<Emp> empList = service.selectRetirement();
	...
	private void updateEmp() {
		...
		try {
			...
			emp = new Emp(); // 변수 재사용
							 // -> 위에 사용한 emp는 쓸모를 다 했기 때문에 재사용할 수 있음.
	...
	private void deleteEmp() {
		...
		try {
			int result = service.deleteEmp(empId, check);
			
			if(result > 0) System.out.println("[회원 정보 삭제 성공]");
...
//		}
		...
		try {
			...
			int check = service.checkEmployee(empId);
			...
			// 'Y'인 경우 서비스 호출  (성공)
			/*int result =*/ service.retireEmp(empId);
	...
	private void hireLately() {
		...
		try {
			List<Emp> empList = service.hireLately();
